...
def stopping_condition(
    ...
    """
    # Compute distances from the best vertex (X[0]) to all others
    distances = np.apply_along_axis(lambda x: norm(x - X[0, :]), 1, X[1:])
    return np.max(distances) < eps

def update_vertices(X: np.ndarray) -> np.ndarray:
    """
    ...
    """
    x0 = X[0, :]
    # Move each vertex halfway towards the best vertex
...
def optimize(
    ...
    while not stopping_condition(X, eps, norm):
